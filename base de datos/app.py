...
def extract():
  ...
  try:
    ...
    session["lote"] = (
        datetime.now().strftime("%y%m%d") + "-" + uuid.uuid4().hex[:4].upper()
    )

    return render_template(
        "extract.html",
        total=len(df),
        tabla=df.to_html(classes="data-table", index=False),
    )
...
def load():
  ...
  try:
    ...
    return render_template(  # Se envian a load.html los resultados de la carga
        ...
        mensajes=resultado[
            "mensajes"
        ],  # Lista de mensajes de cada registro procesado para mostrarlo en la interfaz web
    )
  ...
  try:
    ...
    for f in filas:
      if f.get("fecha_de_recepcion"):
        f["fecha_de_recepcion"] = f["fecha_de_recepcion"].strftime(
            "%Y-%m-%d %H:%M"
        )
...
def api_detalle_paquete(id_paquete):
  ...
  try:
    ...
    if paquete.get("fecha_de_recepcion"):
      paquete["fecha_de_recepcion"] = paquete["fecha_de_recepcion"].strftime(
          "%Y-%m-%d %H:%M"
      )
        ...
        if estado_cambio:
          cursor.execute(
              ...
              (
                  id_paquete,
                  estado_nuevo,
                  "Actualizado manualmente desde el panel",
              ),
          )
...
# =====================================================================
# RUTAS CRUD ADICIONALES PARA POSTMAN (POST, PUT, DELETE sobre /paquetes)
# =====================================================================


# 1. CREAR PAQUETE (POST /paquetes)
@app.route("/paquetes", methods=["POST"])
def crear_paquete_postman():
  datos = request.get_json(force=True, silent=True) or {}

  tracking = datos.get("tracking")
  descripcion = datos.get("descripcion", "")
  peso = datos.get("peso", 0.0)
  estado = datos.get("estado", "recibido en bodega")
  metodo = datos.get("metodo_de_llegada", "aereo")
  id_cliente = datos.get("id_cliente", 1)  # ID de cliente por defecto
  id_categoria = datos.get(
      "id_categoria_producto", 1
  )  # ID categoria por defecto
  id_bodega = datos.get("id_bodega", 1)  # ID bodega por defecto

  if not tracking:
    return jsonify({"error": "El campo 'tracking' es obligatorio"}), 400
  ...
  try:
    ...
    try:
      with conexion.cursor() as cursor:
        sql = """
                    INSERT INTO paquetes (tracking, descripcion, peso, estado, metodo_de_llegada, id_cliente, id_categoria_producto, id_bodega)
    ...
    return (
        jsonify({"ok": True, "mensaje": "Paquete creado exitosamente", "data": datos}),
        201,
    )
...
# 2. ELIMINAR PAQUETE (DELETE /paquetes/<id> o /api/paquetes/<id>)
@app.route("/paquetes/<int:id_paquete>", methods=["DELETE"])
...
def eliminar_paquete(id_paquete):
  try:
    ...
    try:
      with conexion.cursor() as cursor:
        # Primero eliminar eventos de tracking relacionados si existen
        cursor.execute(
            "DELETE FROM tracking_eventos WHERE id_paquete = %s", (id_paquete,)
        )
    ...
    return jsonify({"ok": True, "mensaje": f"Paquete {id_paquete} eliminado"}), 200
